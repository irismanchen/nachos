...
// advance simulated time
    if (status == SystemMode) {
     //   printf("Time Interrupt:timeTicks is %d\n",stats->totalTicks);
...
// check any pending interrupts are now ready to fire
	...
	//currentThread->Yield();
        printf("currentThread \"%s\" timeTicks left is: %d\n",currentThread->getName(),currentThread->timeTicksNum);
        if(currentThread->timeTicksNum<=0)
        {
           currentThread->timeTicksNum=100;
           currentThread->MyYield();
        }
        else{
           currentThread->timeTicksNum-=10;
        }
	status = old;
    }
}
