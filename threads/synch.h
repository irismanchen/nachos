...
class Monitor_PC{
  public:
      Monitor_PC();
      ~Monitor_PC();
      void ProduceInsert(Lock* conditionLock,int item);
      int ConsumerRemove(Lock* conditionLock);
  private:
      Condition* full;
      Condition* empty;
      int buffer[5];
      int low,high,tem,count;
};
class Read_Write_Lock{
  public:
      Read_Write_Lock();
      ~Read_Write_Lock();
      void AcquireReadLock(Lock* rwLock);
      void AcquireWriteLock(Lock* rwLock);
      void ReleaseReadLock(Lock* rwLock);
      void ReleaseWriteLock(Lock* rwLock);
  private: 
      Lock* mutex;
      int rc;
};
