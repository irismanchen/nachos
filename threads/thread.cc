...
Thread::Thread(char* threadName)
{
    ...
    for(int i=0;i<128;i++)
    {
     	if(tId[i]==0)	//if the first unused thread was found,then allocate it
	{
		threadId=i;//mark it as allocated
		userId=1;//set userId
		tId[i]=1;//modify the flag
		break;
	}
    }
...
Thread::MyYield ()
{
    ...
    nextThread = scheduler->FindNextToRun();
    if ((nextThread != NULL)&&(nextThread->getPriority()>this->priority)) {
	scheduler->ReadyToRun(this);
        nextThread->totalTicksRec = stats->totalTicks;
	scheduler->Run(nextThread);
    }
    else if ((nextThread != NULL)&&(nextThread->getPriority()<this->priority)) {
	scheduler->ReadyToRun(nextThread);
        currentThread->totalTicksRec = stats->totalTicks;
        printf("Still Me!\n");
    }
    (void) interrupt->SetLevel(oldLevel);
}
