...
// first, set up the translation 
	...
	pageTable[i].physicalPage = memoryManagement->Find();
        printf("pageTable[%d].physicalPage is %d\n",i,pageTable[i].physicalPage);
	...
	pageTable[i].readOnly = FALSE;  // if the code segment was entirely on 
        ...
        pageTable[i].CreatTime = stats->totalTicks;
...
AddrSpace::~AddrSpace()
{
   for(int i=0; i<numPages;i++){
      memoryManagement->Clear(pageTable[i].physicalPage);
      printf("pageTable[%d].physicalPage %d is clear\n",i,pageTable[i].physicalPage);
   }
   delete pageTable;
}
